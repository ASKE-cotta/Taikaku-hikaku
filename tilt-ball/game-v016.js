...
window.__BUGGY_VERSION='0.16';
var D=window.ROUTE_DATA,A=window.PixelAssets,W=600,H=900;
if(!A)throw new Error('PixelAssets v0.15 not loaded');
...
var running=false,last=performance.now(),gameMinutes=D.initialMinutes,damage=0,invuln=0,impactFx=null,marineMuzzleFx=null;
...
function resetGame(){
  ...
  gameMinutes=D.initialMinutes;damage=0;invuln=0;impactFx=null;marineMuzzleFx=null;currentNode=D.start;currentEdge=null;edgeProgress=0;nextHazard=0;hazards=[];pendingPatterns=[];lastEncounterLabels=[];flowDir=1;terrainHitCooldown=0;encounterIndex=0;
...
function spawnCannon(e){
  ...
  marineMuzzleFx={side:Math.random()<.5?-1:1,t:.30};
...
function hitTerrain(label,pen){
  ...
  impactFx={x:player.x,y:player.y,t:.34};
...
function drawCurrent(){
  ...
  for(var y=176;y<850;y+=112){
    for(var x=-96+shift;x<696;x+=128){
      var xx=dir>0?x:W-x;
      if(!(A.drawCurrentArrow&&A.drawCurrentArrow(ctx,xx,y,dir,1,.72))){
...
function hit(h){
  ...
  impactFx={x:player.x,y:player.y,t:.38};
...
function update(dt){
  ...
  if(impactFx){impactFx.t-=dt;if(impactFx.t<=0)impactFx=null;}
  if(marineMuzzleFx){marineMuzzleFx.t-=dt;if(marineMuzzleFx.t<=0)marineMuzzleFx=null;}
...
function drawSea(){
  ...
  // Sprite-backed water texture when available.
  if(A.drawWaterTile){
    for(var wy=-32+off%32;wy<H;wy+=64){
      for(var wx=16;wx<W;wx+=96)A.drawWaterTile(ctx,wx,wy,1,.34);
    }
  }
  ...
  for(var fy=-100+foamOff;fy<H;fy+=150){
    for(var fx=24;fx<W;fx+=170)A.drawFoam(ctx,fx+((fy/50)%2)*26,fy,(fx+fy)%3);
  }

  // Edge scenery gives the route a sense of place without affecting collisions.
  if(currentEdge&&currentEdge.hazard==='海軍'){
    A.drawMarineShip(ctx,88,112,.82);
    A.drawMarineShip(ctx,512,138,.70);
    if(marineMuzzleFx&&A.drawImpact){
      var mx=marineMuzzleFx.side<0?111:489,my=marineMuzzleFx.side<0?118:143;
      A.drawImpact(ctx,mx,my,.72,clamp(marineMuzzleFx.t/.30,0,1));
    }
  }else if(currentEdge&&currentEdge.hazard==='岩礁'){
    A.drawSmallIsland(ctx,54,118,0);A.drawSmallIsland(ctx,546,148,1);
  }else if(currentEdge&&currentEdge.hazard==='海賊'){
    A.drawPirateShip(ctx,92,126,1);
  }else if(currentEdge&&currentEdge.hazard==='海王類'){
    if(A.drawWave){A.drawWave(ctx,80,120,1,.75);A.drawWave(ctx,490,150,.9,.58);}
  }

  if(currentEdge&&currentEdge.risk>=3){
    A.drawBuoy(ctx,55,216);A.drawBuoy(ctx,545,188);
  }
}
function drawRock(h){
...
function drawCannon(h){
  if(h.state==='warning'){
    ...
    if(!(A.drawCannonTarget&&A.drawCannonTarget(ctx,tx,ty,1))){
    ...
    if(!(A.drawCannonball&&A.drawCannonball(ctx,x,y,1))){
...
function drawPlayer(){
  ...
  if(speed>35){
    A.drawFoam(ctx,player.x-33,player.y+25,0);
    A.drawFoam(ctx,player.x+6,player.y+29,1);
  }
  A.drawPlayerShip(ctx,player.x,player.y,lean,flashState);
}
function drawRouteIdentity(){
  if(!currentEdge)return;
  ctx.save();
  ctx.globalAlpha=.92;
  ctx.fillStyle='rgba(23,43,60,.78)';ctx.fillRect(12,72,190,42);
  ctx.strokeStyle='#FFF1D6';ctx.lineWidth=2;ctx.strokeRect(12,72,190,42);
  if(A.drawHazardIcon)A.drawHazardIcon(ctx,currentEdge.hazard,32,93,26);
  ctx.fillStyle='#FFF1D6';ctx.font='bold 11px monospace';ctx.textAlign='left';
  ctx.fillText('MAIN '+meta(currentEdge.hazard).short,51,91);
  ctx.fillStyle='#F4C84B';ctx.fillText(riskStars(currentEdge.risk),51,107);
  ctx.restore();
}
function draw(){
  drawSea();drawCurrent();drawReefTerrain();hazards.forEach(drawHazard);drawFork();
  if(impactFx&&A.drawImpact){
    var a=clamp(impactFx.t/.38,0,1);
    A.drawImpact(ctx,impactFx.x,impactFx.y,1.15+(1-a)*.5,a);
  }
  drawPlayer();drawRouteIdentity();
...
function drawTitleHero(){
  var c=document.getElementById('heroArt');if(!c||!c.getContext)return;
  var hctx=c.getContext('2d');hctx.imageSmoothingEnabled=false;
  hctx.clearRect(0,0,c.width,c.height);
  hctx.fillStyle='#2786C1';hctx.fillRect(0,0,c.width,c.height);
  if(A.drawWave){A.drawWave(hctx,58,72,.9,.65);A.drawWave(hctx,268,33,.75,.5);}
  A.drawMarineShip(hctx,42,51,.62);
  A.drawMarineShip(hctx,278,53,.62);
  A.drawPlayerShip(hctx,160,53,0,false);
  if(A.drawCloud){A.drawCloud(hctx,100,25,1.1,.8);A.drawCloud(hctx,225,78,.85,.62);}
}
try{window.addEventListener('pixel-sprites-ready',drawTitleHero);}catch(e){}
...
window.__BUGGY_ASSET_MODE=function(){return A.spriteReady&&A.spriteReady()?'SPRITE':'FALLBACK';};
...
resetGame();drawTitleHero();requestAnimationFrame(loop);
})();