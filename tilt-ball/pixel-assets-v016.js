...
try{
  if(typeof Image!=='undefined'){
    img=new Image();
    img.onload=function(){ready=true;window.__PIXEL_SPRITES_READY=true;try{window.dispatchEvent(new Event('pixel-sprites-ready'));}catch(e){}};
...
F.drawWaterTile=function(ctx,x,y,scale,alpha){
  return drawFrame(ctx,'waterTile',x,y,32*(scale||1),32*(scale||1),0,alpha==null?.42:alpha,false);
};
F.drawWave=function(ctx,x,y,scale,alpha){
  return drawFrame(ctx,'wave',x,y,64*(scale||1),16*(scale||1),0,alpha==null?.72:alpha,false);
};
